...
from models.student_quiz_report import (
    CreateStudentQuizReportModel,
    StudentQuizReportController,
)
...
class StudentQuizReportsRouter:
    ...
    def router(self):
        api_router = APIRouter(
            prefix="/student_quiz_reports", tags=["student_quiz_reports"]
        )
        ...
        def create_student_quiz_report(report_data: CreateStudentQuizReportModel):
            return self.__student_quiz_reports_controller.create_student_quiz_report(
                report_data
            )

        @api_router.get("/get")
        def get_student_quiz_report(student_id: str = None, session_id: str = None):
            if student_id is None or session_id is None:
                raise HTTPException(
                    status_code=400,
                    detail="Both student_id annd session_id have to be specified",
                )
            try:
                return self.__student_quiz_reports_controller.get_student_quiz_report(
