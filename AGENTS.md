...
This repo uses [mex](https://github.com/mex-memory/mex) for structured agent memory. **At the start of every session, read `.mex/ROUTER.md`** — it contains the current project state, a routing table for task-specific context (`.mex/context/`), and task runbooks (`.mex/patterns/`). Follow its behavioural contract (CONTEXT → BUILD → VERIFY → DEBUG → GROW). Run `mex check` to detect scaffold drift.

**GROW — after every meaningful piece of work, update the scaffold:**
- **Ground:** name what actually changed (behavior, schema, command, dependency, workflow)
- **Record:** update "Current Project State" in `.mex/ROUTER.md` and any `.mex/context/` file that is now out of date
- **Orient:** if the task can recur, add or update a runbook in `.mex/patterns/` (+ its `INDEX.md` row)
- **Write:** bump `last_updated` in every scaffold file you touched; `mex log "<note>"` when the rationale matters
...
The application supports PDF generation for reports using the `app/utils/pdf_converter.py` module:
